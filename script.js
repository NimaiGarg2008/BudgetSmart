...
let categories = ["Dairy", "Meats", "Vegetables", "Fruits"]
let shoppingList = [];
let budget = 0;
...
function setSpeed(speed) {
  ...
  //     if (!button.closest('.button-row-speed')) { // Ensure not a speed button
...
function updateCart() {
  ...
  for (let i = 0; i < cart.length; i++) {
    ...
    totalPrice += convertedPrice * cart[i].quantity;  // Adjusted to account for the quantity
...
function checkout()
{
  ...
  function badCart(cart, category, list)
  {
    let currentCategory = category[0];
    let checker = [0];
    for (let i = 0; i < cart.length; i++)
    {
      if (currentCategory == cart[i].category)
      {
        checker[checker.length-1] += cart[i].quantity;
      }
      if (currentCategory != cart[i].category)
      {
        if (category[checker.length] != cart[i].category)
        {
          while (category[checker.length] != cart[i].category)
            {
              checker.push(0);
            }
        }
        currentCategory = cart[i].category;
        checker.push(cart[i].quantity);
      }
    }
...
function randomizePricesAndDesirability() {
  const priceRange = 0.2;  // Allow up to 20% change in price
  const desirabilityRange = 0.3;  // Allow up to 30% change in desirability

  items.forEach(item => {
      // Randomize price
      let priceChange = (Math.random() * 2 * priceRange) - priceRange;  // Random change between -20% to +20%
      item.price += item.price * priceChange;

      // Randomize desirability
      let desirabilityChange = (Math.random() * 2 * desirabilityRange) - desirabilityRange;  // Random change between -30% to +30%
      item.desirability += item.desirability * desirabilityChange;

      // Round desirability to 2 decimal points
